...
namespace WireBuilder
{
    ...
    public class WireType : ScriptableObject
    {
        ...
        [Range(3, 12)]
        [Tooltip("Controls the amount of sides on the wire mesh")]
        public int radialSegments = 5;

        //Physical properties
        [Range(0.1f, 10f)]
        [Tooltip("Heavy wires will sag lower")]
        public float weight = 5f;
        [Range(0.01f, 0.5f)]
        [Tooltip("A wire's thickness")]
        public float diameter = 0.1f;

        //Appearance
        public Material material;
        public LineTextureMode textureMode = LineTextureMode.Tile;
        [Tooltip("The distance the texture tiles over the length of a wires. A value of one means the texture is stretched over the wires.")]
        ...
        public bool enableTreeMask = false;
        [Range(1f, 40f)]
        public float treeMaskWidth = 20f;
        [Tooltip("When enabled, a VegetationMaskLine is added to each wire, and removes any large objects underneath them")]
        public bool enableLargeObjectMask = false;
        [Range(1f, 40f)]
        public float largeObjectMaskWidth = 20f;
    }
}